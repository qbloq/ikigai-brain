...
#
# Schema (the org's, per DB_SCHEMA):
...
DROP TABLE IF EXISTS archetype_acceptance_criteria CASCADE;
DROP TABLE IF EXISTS archetype_inputs  CASCADE;
DROP TABLE IF EXISTS archetype_outputs CASCADE;
DROP TABLE IF EXISTS archetype_params  CASCADE;
DROP TABLE IF EXISTS activity_archetypes CASCADE;
DROP TABLE IF EXISTS sops            CASCADE;
DROP TABLE IF EXISTS macro_processes CASCADE;
...
CREATE TABLE sops (
  ...
  macro_process_code text NOT NULL REFERENCES macro_processes(code) ON UPDATE CASCADE,
  name               text NOT NULL,
  owner_roles        text[],
  status             text DEFAULT 'candidate',
  note               text,
...
CREATE TABLE activity_archetypes (
  ...
  verb             text,
  artifact         text,
  sop_code         text REFERENCES sops(code) ON UPDATE CASCADE,
  default_role     text,
...
CREATE TABLE archetype_params (
  id           uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  archetype_id text REFERENCES activity_archetypes(id) ON DELETE CASCADE,
  key          text NOT NULL,
  label        text,
...
CREATE TABLE archetype_acceptance_criteria (
  id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  output_id           uuid REFERENCES archetype_outputs(id) ON DELETE CASCADE,
...
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS archetype_id text;
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS archetype_confidence text;
ALTER TABLE tasks ADD COLUMN IF NOT EXISTS archetype_match_method text;

CREATE INDEX IF NOT EXISTS idx_sops_macro       ON sops(macro_process_code);
CREATE INDEX IF NOT EXISTS idx_archetypes_sop   ON activity_archetypes(sop_code);
CREATE INDEX IF NOT EXISTS idx_tasks_archetype  ON tasks(archetype_id);

-- ---- Seed from catalog JSON ----------------------------------------------
WITH cat AS (SELECT :'catalog'::jsonb AS j)
INSERT INTO macro_processes (code, name, value_chain_order, cadence, owner_roles, owner_gap, status, note)
...
WITH cat AS (SELECT :'catalog'::jsonb AS j)
INSERT INTO sops (code, macro_process_code, name, owner_roles, status, note)
...
WITH cat AS (SELECT :'catalog'::jsonb AS j)
INSERT INTO activity_archetypes (id, slug, name, verb, sop_code, is_gate, status)
...
WITH cat AS (SELECT :'catalog'::jsonb AS j)
INSERT INTO archetype_params (archetype_id, key, position)
...
WITH cat AS (SELECT :'catalog'::jsonb AS j)
INSERT INTO archetype_inputs (archetype_id, title, description, io_type_id, artifact_type_id, is_required, position)
...
WITH cat AS (SELECT :'catalog'::jsonb AS j),
ins_out AS (
  INSERT INTO archetype_outputs (archetype_id, title, description, io_type_id, artifact_type_id, is_required, position)
...
INSERT INTO archetype_acceptance_criteria (output_id, criterion, criterion_category, verification_method, is_required, position)
...
ALTER TABLE tasks
  ...
  FOREIGN KEY (archetype_id) REFERENCES activity_archetypes(id) ON UPDATE CASCADE;

\echo '==== counts ===='
SELECT 'macro_processes' t, count(*) n FROM macro_processes
UNION ALL SELECT 'sops', count(*) FROM sops
UNION ALL SELECT 'activity_archetypes', count(*) FROM activity_archetypes
UNION ALL SELECT 'archetype_params', count(*) FROM archetype_params
UNION ALL SELECT 'archetype_inputs', count(*) FROM archetype_inputs
UNION ALL SELECT 'archetype_outputs', count(*) FROM archetype_outputs
UNION ALL SELECT 'archetype_acceptance_criteria', count(*) FROM archetype_acceptance_criteria
ORDER BY 1;

\echo '==== template io_type integrity (should be 0) ===='
SELECT count(*) AS unresolved_io_types FROM (
  SELECT io_type_id FROM archetype_inputs WHERE io_type_id IS NULL
  UNION ALL SELECT io_type_id FROM archetype_outputs WHERE io_type_id IS NULL
) x;

\echo '==== orphan tasks (archetype_id not in catalog)? ===='
SELECT count(*) AS orphan_tasks FROM tasks t
