...
tid="$(psql_ro -t -A -c "SELECT id FROM tasks WHERE id::text LIKE '${idarg}%' LIMIT 2;" | head -1)"
...
if [[ "$FORMAT" == "json" ]]; then
  ...
  SELECT row_to_json(x) FROM (
    SELECT t.*,
      (SELECT json_agg(i) FROM task_inputs  i WHERE i.task_id=t.id) AS inputs,
      (SELECT json_agg(o) FROM task_outputs o WHERE o.task_id=t.id) AS outputs,
      (SELECT json_agg(c) FROM task_acceptance_criteria c
         WHERE c.output_id IN (SELECT id FROM task_outputs WHERE task_id=t.id)) AS criteria,
      (SELECT json_agg(d) FROM task_todos d WHERE d.task_id=t.id) AS todos,
      (SELECT json_agg(m) FROM task_comments m WHERE m.task_id=t.id) AS comments
...
psql_ro -c "SELECT title, is_required, is_satisfied, it.display_name AS io_type
  FROM task_inputs i LEFT JOIN io_types it ON it.id=i.io_type_id
  WHERE i.task_id='$tid' ORDER BY i.position;"

echo "== OUTPUTS =="
psql_ro -c "SELECT title, is_required, is_delivered, it.display_name AS io_type
  FROM task_outputs o LEFT JOIN io_types it ON it.id=o.io_type_id
...
psql_ro -c "SELECT c.criterion, c.verification_method AS method, c.is_required, c.is_met
  ...
  WHERE c.output_id IN (SELECT id FROM task_outputs WHERE task_id='$tid')
  ORDER BY c.position;"

echo "== TODOS =="
psql_ro -c "SELECT completed, text FROM task_todos WHERE task_id='$tid' ORDER BY position;"

echo "== COMMENTS =="
psql_ro -c "SELECT to_char(created_at,'YYYY-MM-DD') AS date, author_name, text
  FROM task_comments WHERE task_id='$tid' ORDER BY created_at;"
