...
-- Schema: ikigaigm.  Applied: 2026-06-28.
--
...
-- to ikigaigm. With search_path=ikigaigm,public an unqualified `sops` resolves to
-- ikigaigm.sops.
--
...
-- ---------------------------------------------------------------- macro_processes
CREATE TABLE IF NOT EXISTS ikigaigm.macro_processes (
...
-- ------------------------------------------------------------------------- sops
CREATE TABLE IF NOT EXISTS ikigaigm.sops (
  code               text PRIMARY KEY,                -- 'S2.1'
  macro_process_code text NOT NULL
                       REFERENCES ikigaigm.macro_processes(code) ON UPDATE CASCADE,
  name               text NOT NULL,
  owner_roles        text[],
...
-- ----------------------------------------------------------- activity_archetypes
CREATE TABLE IF NOT EXISTS ikigaigm.activity_archetypes (
...
-- ------------------------------------------------------------- archetype_params
CREATE TABLE IF NOT EXISTS ikigaigm.archetype_params (
...
CREATE TABLE IF NOT EXISTS ikigaigm.archetype_outputs (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  archetype_id     text REFERENCES ikigaigm.activity_archetypes(id) ON DELETE CASCADE,
  title            text NOT NULL,
  description      text,
...
CREATE TABLE IF NOT EXISTS ikigaigm.archetype_inputs (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  archetype_id     text REFERENCES ikigaigm.activity_archetypes(id) ON DELETE CASCADE,
  title            text NOT NULL,
  description      text,
...
CREATE TABLE IF NOT EXISTS ikigaigm.archetype_acceptance_criteria (
  id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE ikigaigm.tasks ADD COLUMN IF NOT EXISTS archetype_match_method text;  -- rule|embedding|llm|human
