...
-- Schema: ikigaigm.  Applied: 2026-07-04.
--
...
--     source_meeting_id  uuid  -- FK → ikigaigm.meetings(id)  (when type='meeting')
...
ALTER TABLE ikigaigm.tasks ADD COLUMN IF NOT EXISTS source_type        text;
ALTER TABLE ikigaigm.tasks ADD COLUMN IF NOT EXISTS source_meeting_id  uuid;
ALTER TABLE ikigaigm.tasks ADD COLUMN IF NOT EXISTS source_url         text;
ALTER TABLE ikigaigm.tasks ADD COLUMN IF NOT EXISTS source_external_id text;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tasks_source_meeting_id_fkey') THEN
    ALTER TABLE ikigaigm.tasks
      ...
      FOREIGN KEY (source_meeting_id) REFERENCES ikigaigm.meetings(id) ON UPDATE CASCADE;
  END IF;
END $$;

-- Lookups: dedup by Notion page id, filter by origin, join back to meetings.
CREATE INDEX IF NOT EXISTS idx_tasks_source_external ON ikigaigm.tasks(source_external_id);
CREATE INDEX IF NOT EXISTS idx_tasks_source_type     ON ikigaigm.tasks(source_type);
CREATE INDEX IF NOT EXISTS idx_tasks_source_meeting  ON ikigaigm.tasks(source_meeting_id);
