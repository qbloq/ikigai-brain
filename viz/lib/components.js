...
function validateSpecV2(spec) {
  ...
  if (!pat) {
    errors.push(`Patrón desconocido: «${spec.pattern}» — esta UI requiere actualizar el núcleo.`);
  ...
  for (const [name, req] of Object.entries((pat.manifest && pat.manifest.slots) || {})) {
    ...
    else if (!blk) errors.push(`Bloque desconocido en ${name}: «${filler.block}» — requiere actualizar el núcleo.`);
...
function validateSpec(spec) {
  ...
  if (!page) errors.push(`Componente desconocido: «${comp}» — esta UI requiere actualizar el núcleo.`);
