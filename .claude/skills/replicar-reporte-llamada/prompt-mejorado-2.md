...
Anchors for BUDGET, AUTHORITY and TIMELINE (do NOT apply them to Need):
...
NEED — use the anchors below INSTEAD of the ones above. Need has its own axis
because the anchors above cannot fail on it: booking a sales call about this
product IS an explicit, quotable statement of wanting the outcome, so every
lead would satisfy them and the item would carry no information.

Score the COST OF INACTION, not the strength of the stated desire. Wanting the
outcome is the price of entry to this call and is worth zero points on its own.
However well a lead articulates their goal, articulation is not need.
- 90-100: The status quo is actively costing them, they said so with a number
  or a deadline, AND they already spent money or time trying to fix it and it
  did not work.
- 70-89:  A concrete problem with a date or an amount attached, but no prior
  attempt to solve it — or an attempt that is still running and undecided.
- 40-69:  A real goal with no cost of delay. The lead states they are doing
  fine and want better. Aspiration without pressure lives here no matter how
  clearly or how emotionally it is expressed.
- 10-39:  The need was proposed to them by someone else, or the lead cannot
  name what breaks if nothing changes.
- 0:      No evidence at all in the transcript. If you score 0, the analysis
  field MUST say explicitly that the item was not discussed.

Two failure modes to avoid on Need, both of which inflate the score:
- A painful past (a loss, a bankruptcy, a bereavement) is evidence of need ONLY
  if it is still costing them today. Pain that has been absorbed is history,
  not need.
- Fluency is not urgency. A lead who names their goal in figures and timelines
  is easy to score high and often has no pressure to act at all.
...
Need: [Score/100] - [State the cost of inaction you scored on, and any prior attempt to solve it. If there is no cost of delay, say so explicitly.]
