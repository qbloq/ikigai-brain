...
- **project** ← the **per-task project from the sidecar** (`project` column), NOT
  the meeting's project. Team meetings (mostly tagged with the org's internal
  project) span clients: route each task to the client projects by content,
  keep only genuinely internal work under the internal project, and ask on `⚠️ undecided`.
...
- **external collaborators** (people not in the org roster)
...
content = el proyecto del cliente. If a nickname is unknown, ask — don't guess.
