...
## 3. Data Model (Supabase — `ikigaigm` schema)
...
- **`add_notion_artifact.sql`** widens the resolver CHECK to include `notion` and upserts the `notion_page` type — apply to `ikigaigm` to make the 8th type live.
