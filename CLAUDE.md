...
| `relacionadas.sh --titulo T --project P [--archetype A] [--assignee N] [--ids a,b] [--limit N] [--json]` | Tareas del cerebro relacionadas con una propuesta (o cualquier título), cada fila con `priority`, `motivo` y `score` — señales **sumadas** (puede pasar de 100): citada en `--ids` (100) · mismo arquetipo+proyecto (+40) · mismo dueño (+20) · palabras del título (Jaccard ×40). Abiertas antes que cerradas. Read-only. Fuente viz `tareas_relacionadas` (bloque «Relacionadas» de la UI Revisión de propuestas). |
| `sin_arquetipo.sh [--project P] [--open] [--json]` | Tareas con `archetype_id IS NULL` + arquetipo **propuesto** (top-3): heurística léxica contra el catálogo (verbo/nombre/SOP) sumada al voto de las tareas vecinas ya etiquetadas (Jaccard sobre el título). `score` 0-100; bajo 25 declara `sugerido=null` antes que inventar. Read-only. Fuente viz `tareas_sin_arquetipo`. |
...
| `aplicar_arquetipos.sh [--dry-run] [--json]` **[WRITE]** | Gemelo de `crear_de_propuestas.sh` para las marcas de arquetipo: por cada fila de `arquetipos` (sqlite local) con decisión ≠ `ninguno` y sin `aplicado_en`, corre `set_archetype.sh <task> <A> --method human` y sella `aplicado_en`. ⚠️ Mueve el puntero, no reescribe el contrato IO. |
...
| `propuestas_backups.sh [--desde D] [--json]` | Read-only. Lista los `backups/meeting-tasks/*.json` desde `--desde` (default 2026-08-21) con la marca `cargado` (ya tiene lote en la sqlite). Fuente viz `propuestas_backups` (barra superior de la UI Revisión de propuestas). |
| `propuesta_cargar.sh <meeting-id\|prefijo> [--json]` **[WRITE]** | Cargar un backup: lee el gemelo `.json`, valida cada contrato §A con `create_task.sh --dry-run`, inserta lote+filas en una txn. Se niega si el lote ya existe — un backup se carga una sola vez. |
