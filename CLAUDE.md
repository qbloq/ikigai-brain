...
La Cola de Gobernanza de [docs/torre-de-control.md](docs/torre-de-control.md)
(T2): **cola = derivado − decidido**. Lo pendiente se deriva en vivo (scan
sobre `data/forks/`); las decisiones son eventos append-only en
[forja/gobernanza/decisiones.jsonl](forja/gobernanza/README.md), commiteados
...
- **Routes**: `GET /` (shell, `?ui=<id>` opens one) · `GET /u/:id` (standalone page)
  ...
  la iframea el panel de la Cola de Gobernanza) · `GET /health`.
