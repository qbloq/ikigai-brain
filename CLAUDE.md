<!-- PLANTILLA DEL NÚCLEO — este CLAUDE.md se instancia al nacer un cerebro:
     los {slots} se sustituyen con los datos de la org (docs/deltas-architecture.md
     define la mecánica de capas; la identidad vive en cerebro.json). -->

- The DB connection string is DATABASE_URL in .env
- We only use the ´{schema}´ schema
...
to the `{schema}` schema, and evaluate dates in `America/Bogota`. Every script
...
- **DB tables** (`{schema}`, seeded from the JSON): `macro_processes`, `sops`
...
## Tasks data model (schema `{schema}`)
